...
else 
{
	...
	while (!feof($fp)) 
	{
		...
		if (strcmp ($res, "VERIFIED") == 0) 
		{
			...
			$paypal = strip_tags($_POST['receiver_email']);
			$txnid = strip_tags($_POST['txn_id']);
			$custom = explode("|",$_POST['custom']);
			$packageid = $custom[0];
			$username = $custom[1];
			$status = strip_tags($_POST['payment_status']);
			...
			$checksettings = mysqli_query($con,"SELECT * FROM `settings`");

			while ($row = mysqli_fetch_array($checksettings)) 
			{
				$realpaypal = $row['paypal'];
			}

			$checkpackage = mysqli_query($con,"SELECT * FROM `packages` WHERE `id` = '$packageid'");
